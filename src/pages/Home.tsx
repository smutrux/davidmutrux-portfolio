  ...
  return (
    <div className="relative">
      ...
      <section className="max-w-7xl mx-auto px-6 py-32 grid grid-cols-1 md:grid-cols-3 gap-12">
        {[
          { title: 'Sound', path: '/sound', desc: 'Immersive audio landscapes and precision recording.' },
          { title: 'Production', path: '/production', desc: 'Seamless execution from concept to final delivery.' },
          { title: 'Camera', path: '/camera', desc: 'Visual storytelling through light and composition.' },
        ].map((dept) => (
          <div key={dept.title}>
            <Link to={dept.path} className="group block">
              <h2 className="text-3xl font-bold mb-4 transition-all duration-500">{dept.title}</h2>
              <p className="text-accent/40 text-sm leading-relaxed mb-6">{dept.desc}</p>
